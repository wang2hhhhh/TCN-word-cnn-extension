...
import json
sys.path.append("../../")
from TCN.word_cnn_distilBERT.utils import *
from TCN.word_cnn_distilBERT.model import *
import pickle
...
parser.add_argument('--out_dir', type=str,
                    help='output_dir'),
parser.add_argument('--bertmodel', type=str, default = "distilbert-base-uncased",
                    help='output_dir')
...
with open(os.path.join(args.out_dir, 'args'), 'w') as outfile:
  json.dump(args.__dict__, outfile, indent=4)

corpus = data_generator(args)
eval_batch_size = 32
train_data = batchify(corpus.train, args.train_batch_size, args)
val_data = batchify(corpus.valid, args.eval_batch_size, args)
test_data = batchify(corpus.test, args.eval_batch_size, args)


n_words = len(corpus.tokenizer)
if args.bertmodel == "bert-base-uncased" or args.bertmodel =='distilbert-base-uncased':
  embsize = 768
  
num_chans = [args.nhid] * (args.levels - 1) + [embsize]
k_size = args.ksize
dropout = args.dropout
emb_dropout = args.emb_dropout
# tied = args.tied
model = TCN(embsize, n_words, num_chans, dropout=dropout, emb_dropout=emb_dropout, kernel_size=k_size)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if val_loss < best_vloss:
                with open(os.path.join(args.out_dir, "model.pt"), 'wb') as f:
                    print('Save model!\n')
                    torch.save(model, f)
                best_vloss = val_loss

            # Anneal the learning rate if the validation loss plateaus
            if epoch > 5 and val_loss >= max(loss_dict['all_vloss'][-5:]):
                lr = lr / 2.
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr

            loss_dict['all_vloss'].append(val_loss)
            loss_dict['all_tloss'].append(test_loss)
            loss_dict['all_vppl'].append(val_ppl)
            loss_dict['all_tppl'].append(test_ppl)

            with open(os.path.join(args.out_dir, 'loss_dict'), 'w') as outfile:
    ...
    with open(os.path.join(args.out_dir, "model.pt"), 'rb') as f:
        model = torch.load(f)
    ...
    with open(os.path.join(args.out_dir, 'final_test_loss'), 'w') as outfile:
      json.dump(test_loss, outfile, indent=4)
